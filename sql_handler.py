...
class SQLHandler:
	...
	@allow_method
	def execute_scalar(self, sql_statement: str, *parameters):
		"""
		Purpose: Execute a query or stored procedure and return a single scalar value.
		"""
		conn = self._conn()
		with conn:
			with conn.cursor() as cursor:
				...
				return None
	@allow_method
	def execute_statements(self, sql_statements: List[str], *parameters) -> None:
		"""
		Purpose: Perform any Create, Update, Insert, Delete operation.
	...
	@allow_method
	def iter_execute(self, parameterized_sql_statement: str, rows, error_handling ='raise', debug=False, table_name=None, key_col=None) -> None:
		"""
		...
		Required arguments:
			...
			rows (iterable): The data to insert or update. Must be an iterable of tuples.
				Each tuple in iterable must contain number of rows that equal the number 
		...
		if debug and (error_handling !='raise' or table_name is None or key_col is None):			
			raise Exception('Debug mode requires error_handling="raise", table_name, AND key_col specified.')
				...
				for row in rows:
					try:
						...
						if debug:
							...
							# Attempt to insert just the key to see if key-level violations exist
							c.execute(f'INSERT INTO {table_name} ({key_col}) VALUES (?);', (key_value,))
							for field, value in zip(row._fields, row):
								if field != key_col:
									print(field, value)
									try:
										c.execute(f'UPDATE {table_name} SET {field} = ? WHERE {key_col} = ?;', (value, key_value))
									except Exception as ex:
										print('\nError:\n\nProblematic field and value:\n\n')
		...
		elif error_handling == 'ignore': # commit each row and ignore exceptions
...
class SQLAlchemyHandler(SQLHandler):
